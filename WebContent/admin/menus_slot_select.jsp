...
<main class="content">
  ...
  <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:16px;">
    <div class="card">
      ...
      <a class="button" href="<%=ctx%>/admin/menus_new/edit?dayId=${menuDay.id}&slot=BREAKFAST">作成/編集</a>
    </div>

    <div class="card">
      <h3>昼食</h3>
      <p><c:choose><c:when test="${not empty meals.LUNCH}">登録済み</c:when><c:otherwise>未登録</c:otherwise></c:choose></p>
      <a class="button" href="<%=ctx%>/admin/menus_new/edit?dayId=${menuDay.id}&slot=LUNCH">作成/編集</a>
    </div>

    <div class="card">
      <h3>夕食</h3>
      <p><c:choose><c:when test="${not empty meals.DINNER}">登録済み</c:when><c:otherwise>未登録</c:otherwise></c:choose></p>
      <a class="button" href="<%=ctx%>/admin/menus_new/edit?dayId=${menuDay.id}&slot=DINNER">作成/編集</a>
    </div>
  </div>
